...
//
//
//         // Chia danh sách sinh viên thành các nhóm nhỏ (ví dụ: 5 sinh viên mỗi lần gọi API)
//         // const studentChunks = chunkArray(rows.slice(1), 10);
//         // const limit = pLimit(1); // Giới hạn 10 yêu cầu đồng thời
//         //
//         // const promises = studentChunks.map(chunk => {
//         //     const newData = {
//         //         question: rows[0][7], // Câu hỏi từ dòng đầu tiên, cột 7 (H)
...
//         //         }))
//         //     };
//         //     // Sử dụng p-limit để điều phối các lần gọi API
...
async function callApi(data) {
    const prompt = `
            ...
            **Bài làm của sinh viên:**
            ...
            ---
            YÊU CẦU:
            Hãy nhận xét các tiêu chí chi tiết và chấm điểm bài luận theo các tiêu chí dưới đây bằng tiếng việt. Bài luận có ba đoạn văn, cần được đánh giá theo rubric sau và phản hồi theo đúng format:      
            1. Nội dung (3 điểm):
              +3đ: Bám sát đề tài, ý tưởng rõ ràng và đa dạng
              +2đ: Bám sát nhưng ý trùng lặp, dẫn chứng hạn chế
              +1đ: Ý mờ nhạt, có đoạn lạc đề
